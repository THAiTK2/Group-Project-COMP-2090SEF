...
class TeamManagementSystem:
    ...
    def update_teacher_profile(
        ...
        teacher = self.teachers.get(member_id)
        if teacher is None:
            raise ValueError(f"Teacher ID '{member_id}' not found.")
        ...
        if name is not None:
            ...
            teacher.name = normalized_name

        if department is not None:
            normalized_department = department.strip()
            if not normalized_department:
                raise ValueError("Department cannot be empty.")
            teacher.department = normalized_department

        self.repository.update_teacher(
            member_id=member_id,
            name=teacher.name,
            department=teacher.department,
        )
        ...
        if new_password is not None:
            ...
            account = self.repository.find_account_by_role_member("teacher", member_id)
            if account is None:
                raise ValueError("No login account found for teacher.")
            self.repository.update_account_password(
                username=account["username"],
                new_password=password_value,
            )
        ...
        self._refresh_state()
        return self.teachers[member_id]
    ...
    def update_competition_record(
        ...
        current_competition_name = str(row["competition_name"])
        current_result = str(row["result"]).strip().lower()
        current_opponent = str(row["opponent"])

        year_text = None if year is None else str(year).strip()
        month_text = None if month is None else str(month).strip()
        competition_name_text = (
            None if competition_name is None else str(competition_name).strip()
        )
        result_text = None if result is None else str(result).strip().lower()
        opponent_text = None if opponent is None else str(opponent).strip()

        try:
            merged_year = (
                current_year
                if (year_text is None or year_text == "")
                else int(year_text)
            )
        ...
        try:
            merged_month = (
                current_month
                if (month_text is None or month_text == "")
                else int(month_text)
            )
        except ValueError as error:
            raise ValueError("Month must be an integer.") from error
        merged_competition_name = (
            current_competition_name
            if (competition_name_text is None or competition_name_text == "")
            else competition_name_text
        )
        merged_result = (
            current_result
        ...
        merged_opponent = (
            current_opponent
            if (opponent_text is None or opponent_text == "")
            else opponent_text
        )

        if not 2000 <= merged_year <= 2100:
            raise ValueError("Year must be between 2000 and 2100.")
        if not 1 <= merged_month <= 12:
            raise ValueError("Month must be between 1 and 12.")
        if not merged_competition_name:
            raise ValueError("Competition name cannot be empty.")
        if merged_result not in {"win", "loss", "draw"}:
            raise ValueError("Result must be 'win', 'loss', or 'draw'.")
        if not merged_opponent:
            raise ValueError("Opponent cannot be empty.")

        self.repository.update_competition_record_content(
            record_id=normalized_record_id,
            year=merged_year,
            month=merged_month,
            competition_name=merged_competition_name,
