...
class StaffDashboardFrame(ctk.CTkFrame):
    ...
    def _build_update_tab(self):
        ...
        update_teacher_box = ctk.CTkFrame(frame)
        update_teacher_box.grid(row=1, column=1, padx=8, pady=8, sticky="nsew")
        ctk.CTkLabel(update_teacher_box, text="Update Teacher", font=ctk.CTkFont(weight="bold")).pack(pady=6)

        self.up_teacher_id = self._labeled_entry(update_teacher_box, "Member ID")
        self.up_teacher_name = self._labeled_entry(update_teacher_box, "New Name (optional)")
        self.up_teacher_dept = self._labeled_entry(update_teacher_box, "New Department (optional)")
        self.up_teacher_pass = self._labeled_entry(update_teacher_box, "New Password (optional)", show="*")
        ctk.CTkButton(
            update_teacher_box,
            text="Update Teacher",
    ...
    def _clear_update_teacher_form(self):
        # Clear Update Teacher inputs in Update tab.
        self.up_teacher_id.delete(0, tk.END)
        self.up_teacher_name.delete(0, tk.END)
        self.up_teacher_dept.delete(0, tk.END)
        self.up_teacher_pass.delete(0, tk.END)
    ...
    def _update_teacher(self):
        ...
        def action():
            self.app.system.update_teacher_profile(
                member_id=self.up_teacher_id.get().strip(),
                name=self._or_none(self.up_teacher_name.get().strip()),
                department=self._or_none(self.up_teacher_dept.get().strip()),
                new_password=self._or_none(self.up_teacher_pass.get().strip()),
            )

        self._safe_call(
            action,
            "Teacher updated.",
            on_success=self._clear_update_teacher_form,
        )
