...
# we stop early with clear message so users know how to install missing package.
try:
...
# ---------------- Application root ----------------
# MSUTMSApp is the top-level window and frame router.
class MSUTMSApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        # Container holds all pages; we raise one frame at a time (simple page navigation).
...
class StaffDashboardFrame(ctk.CTkFrame):
    def __init__(self, parent, app):
        ...
        # Tab-based layout keeps functions grouped and easier for first-year demo.
    ...
    def _build_reports_tab(self):
        # Reports tab uses scrollable frame so long content can still be viewed.
    ...
    def _build_sql_tab(self):
        # SQL tab is read-only; used to inspect database state in table form.
        frame = self.tab_sql
    ...
    def _labeled_entry(parent, label_text, show=None):
        # Helper to reduce repeated label+entry code in forms.
    ...
    def _create_treeview(parent, height=10, expand=True):
        # Treeview + vertical/horizontal scrollbars to simulate SQL/Excel-like grid.
    ...
    def _add_coach(self):
        # Similar to student add flow, with optional login account.
        def action():
    ...
    def _update_coach(self):
        # Update coach profile and optional password.
        def action():
